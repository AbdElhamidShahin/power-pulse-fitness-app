...
abstract class AppRouter {
  ...
  //   2. If authenticated → restore cloud data → home (authenticated mode)
  //   3. If not authenticated:
  //      a. If mode == guest → home (guest mode)
  //      b. If mode == none  → entry choice screen
  //
  static Future<String> _initialLocation() async {
    ...
    if (currentUser != null) {
      // Authenticated — refresh local cache from cloud
      try {
        ...
        // Network failure is ok — local cache is used as fallback
      }
    ...
    if (mode == UserMode.authenticated) {
      // Was authenticated but session expired → show entry screen
